...
from chatgpt.nn.llama.rm import LlamaHFRewardModel,LlamaHFRewardModel_Mix
from transformers import LlamaModel, LlamaTokenizer, LlamaConfig
from fengshen_inner.models.megatron import mpu
from chatgpt.nn.llama import LlamaFSRewardModel, LlamaFSRewardModel_Mix
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--multi_value_head", action="store_true"
    )

    args = parser.parse_args()
    if not args.multi_value_head:
        hf_model = LlamaHFRewardModel.from_pretrained(args.pretrained_path)
    else:
        hf_model = LlamaHFRewardModel_Mix.from_pretrained(args.pretrained_path)

    tokenizer = LlamaTokenizer.from_pretrained(args.tokenizer_path)
    hf_config = hf_model.model.config
    ...
    if not args.multi_value_head:
        # default
        fs_model = LlamaFSRewardModel(fs_config)
    else:
        # multi value head
        fs_model = LlamaFSRewardModel_Mix(fs_config)
    ...
    #value_head
    fs_model.value_head.load_state_dict(
        {"weight": hf_model.value_head.weight,
        "bias":hf_model.value_head.bias}
    )
    if args.multi_value_head:
        fs_model.token_value_head.load_state_dict(
            {"weight": hf_model.token_value_head.weight,
            "bias":hf_model.token_value_head.bias}
        ) 

    fs_model.llama.resize_token_embeddings(find_closest_multiple(fs_model.config.vocab_size, int(args.multiplier)))
