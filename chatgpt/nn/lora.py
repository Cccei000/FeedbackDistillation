...
class LoRAModule_Fenshen(nn.Module):
    """A LoRA module base class. All derived classes should call `convert_to_lora()` at the bottom of `__init__()`.
    This calss will convert all torch.nn.Linear layer to LoraLinear layer.
    ...
    def convert_to_lora(self) -> None:
        if self.lora_rank <= 0:
            return
        logging_rank_0("Using LORA, rank = ", self.lora_rank, " !", "debug")
        convert_to_lora_recursively_fenshen(self, self.lora_rank)
        lora.mark_only_lora_as_trainable(self, self.lora_train_bias)
...
def lora_linear_wrapper_fenshen(linear:mpu.ColumnParallelLinear, lora_rank: int):
    assert lora_rank <= linear.input_size, f'LoRA rank ({lora_rank}) must be less than or equal to in features ({linear.in_features})'
    lora_linear = LoraColumnParallelLinear(linear, r=lora_rank, merge_weights=False)
    return lora_linear

def convert_to_lora_recursively_fenshen(module: nn.Module, lora_rank: int) -> None:
    for name, child in module.named_children():
        # if isinstance(child, nn.Linear):
        if name=="query_key_value":
            setattr(module, name, lora_linear_wrapper_fenshen(child, lora_rank))
        else:
            convert_to_lora_recursively_fenshen(child, lora_rank)

class LoraColumnParallelLinear(lora.LoRALayer, nn.Module):
    """Replace in-place ops to out-of-place ops to fit gemini. Convert a torch.nn.Linear to LoraLinear.
    """

    def __init__(
        self,
        base_model: mpu.ColumnParallelLinear,
        # weight: nn.Parameter,
        # bias: Optional[nn.Parameter],
        r: int = 0,
        lora_alpha: int = 1,
        ...
        out_features, in_features = base_model.weight.shape
        self.in_features = in_features
        self.out_features = out_features
        ...
        # Actual trainable parameters
        if r > 0:
            self.lora_A = nn.Parameter(self.base_model.weight.new_zeros((r, in_features)).to(base_model.weight.device))
            self.lora_B = nn.Parameter(self.base_model.weight.new_zeros((out_features, r)).to(base_model.weight.device))
            self.scaling = self.lora_alpha / self.r
            # Freezing the pre-trained weight matrix
            self.base_model.weight.requires_grad = False
        self.reset_parameters()
        if fan_in_fan_out:
            self.base_model.weight.data = self.base_model.weight.data.T
    ...
    def train(self, mode: bool = True):
        ...
        if self.merge_weights and self.merged:
            # Make sure that the weights are not merged
            if self.r > 0:
                self.base_model.weight.data -= T(self.lora_B @ self.lora_A) * self.scaling
            self.merged = False
    ...
    def forward(self, x: torch.Tensor):
        ...
        output, output_bias = self.base_model.forward(x)
        if self.r > 0 and not self.merged:
            if self.r > 0:
