...
class BaichuanAttention(torch.nn.Module):
    ...
    def forward(
        ...
        bsz, q_len, _ = hidden_states.size() # (bs, seq, h)

        proj = self.W_pack(hidden_states) # (bs, seq, 3*h)
        proj = (
            proj.unflatten(-1, (3, self.hidden_size)) # (bs, seq, 3, h)
            .unsqueeze(0)   # (1, bs, seq, 3, h)
            .transpose(0, -2) # (3, bs, seq, 1, h)
            .squeeze(-2) # (3, bs, seq, h)
        )
        query_states = (
            proj[0].view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
        ) # (bs, seq, num_heads, head_dim)
        key_states = (
