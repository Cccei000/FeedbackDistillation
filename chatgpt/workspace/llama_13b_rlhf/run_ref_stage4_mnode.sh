...
MODEL_NAME=PPO_LLAMA_13B_STAGE4_1400
...
# 初始Policy model的路径
POLICY_MODEL_PATH="/cognitive_comp/wanghao/models/llama_sft/llama_stage4_step1400_MP_8"
POLICY_TOKENIZER_PATH="/cognitive_comp/wanghao/models/llama_sft/llama_stage4_step1400_MP_8"
...
# srun -N $NNODES --gres=gpu:$GPUS_PER_NODE --ntasks-per-node=1 --cpus-per-task=180 -o outputs/%x-%j.log -e outputs/%x-%j.err bash -c '$LAUNCHER  --node_rank=$NODE_RANK $CMD'
# srun -N $NNODES --gres=gpu:$GPUS_PER_NODE --ntasks-per-node=1 --cpus-per-task=30 -o outputs/%x-%j.log -e outputs/%x-%j.err bash -c 'echo ${SLURM_PROCID}'
# srun -N $NNODES --gres=gpu:$GPUS_PER_NODE --ntasks-per-node=$GPUS_PER_NODE --cpus-per-task=30 -o outputs/%x-%j.log -e outputs/%x-%j.err bash -c 'echo $((${SLURM_PROCID}))'
# srun python3 -m torch.distributed.run --nproc_per_node $GPUS_PER_NODE --nnodes $NNODES ${CODE_PATH}/${CODE_NAME} $options
# singularity exec --nv \
