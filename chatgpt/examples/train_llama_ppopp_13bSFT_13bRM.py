...
from chatgpt.trainer import PPOPPTrainer
...
def add_neox_ppo_pipeline_args(parent_args:argparse.ArgumentParser):
    ...
    group = parent_args.add_argument_group("PPOPP Args")
    group.add_argument("--ppopp_beta", type=float, default=0.2)
    group.add_argument("--ppopp_beta_decay", type=float, default=0.98)
    group.add_argument("--ppopp_guidance_rate_init", type=float, default=0.8)
    group.add_argument("--ppopp_guidance_rate_decay", type=float, default=0.98)
    group.add_argument("--use_guide_action", action="store_true", default=False)
...
def load_dateset(args):
    ...
    guides = list(dataset["guide"]) if "guide" in dataset.column_names else [""] * dataset.shape[0]
...
def launch(args):
    ...
    if args.use_guide_action:
        print_rank_0("Use Guide Action.")
    else:
        print_rank_0("NOT Use Guide Action.")
    ...
    experience_maker = PPOPPExperienceMaker(
        ...
        use_guide_action=args.use_guide_action,
        ...
        token_level_reward=args.token_level_reward,
        mixed_sample_level_reward=args.mix_reward,
        logger=logger,
    )
    ...
    for task_name, queries in free_task_to_prompts.items():
        free_task_info += f"\n\t{task_name}: {len(queries)}"
    for task_name, queries in guide_task_to_prompts.items():
        guide_task_info += f"\n\t{task_name}: {len(queries)}"
    
    print_rank_0("#"*50)
    print_rank_0(free_task_info)
    print_rank_0(guide_task_info)
    print_rank_0(f"Validate every {args.val_check_interval} episode.")
    print_rank_0("#"*50)
    ...
    ppo_trainer.fit(
        ...
        guidance_init_rate=args.ppopp_guidance_rate_init,
        guidance_rate_decay=args.ppopp_guidance_rate_decay,
        seed=args.seed + mpu.get_data_parallel_rank(),
