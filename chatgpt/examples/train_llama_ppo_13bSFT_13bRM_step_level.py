...
def add_neox_ppo_pipeline_args(parent_args:argparse.ArgumentParser):
    ...
    group.add_argument("--step_level_reward", action="store_true", default=False)
    group.add_argument("--enable_step_ppo", action="store_true", default=False)
    group.add_argument("--enabling_bon", action="store_true", default=False)
    ...
    group.add_argument("--actor_lr", type=float, default=1e-6)
    group.add_argument("--critic_lr", type=float, default=1e-6)
    ...
    group.add_argument("--best_of_n_times", type=int, default=2)
    group.add_argument("--min_step_lengths", type=int, default=0)
...
def get_dataloader_build_func(args, tokenizer:LlamaTokenizer):
    def build_dataloader(replay_buffer, episode, timestep):
        # 数据并行 fine-grained_ppo 处理训练集samples一致
        if args.enable_step_ppo:
            ...
            # print("build_dataloader len: ", sample_count, len(replay_buffer))
        dataset = ReplayBufferDataset(
            replay_buffer, pad_token_id=tokenizer.pad_token_id, enable_gae=args.enable_gae, gamma=args.gamma, lam=args.lam, enable_step_ppo=args.enable_step_ppo
        )
        ...
        for item in dataset:
            ...
            if args.enable_gae and not args.enable_step_ppo:
...
def get_val_experience_saving_func(args, tokenizer:LlamaTokenizer):
    def save_val_experience(replay_buffer, episode, task_name):
        dataset = ReplayBufferDataset(replay_buffer, pad_token_id=tokenizer.pad_token_id, enable_gae=False, gamma=args.gamma, lam=args.lam)
        ...
        # print("val dataset task name:", len(task_name), len(gen_data))
        task_name = task_name[:min(args.buffer_limit_size, len(task_name))]
        assert len(task_name) == len(gen_data) # fg_ppo multi samples for one prompt
...
def launch(args):
    ...
    critic = LlamaCritic(
        ...
        return_mean=(not args.enable_gae) or (args.enable_step_ppo)
    ).to(dtype=torch.bfloat16)
    ...
    experience_maker = LocalInferExperienceMaker(
        ...
        enable_step_ppo=args.enable_step_ppo,
        enable_reward_scaling=args.enable_reward_scaling,
        reward_scaling_gamma=args.gamma,
        step_level_reward=args.step_level_reward,
        gs_args=GSArgs(gs_eval_batch_size=args.gs_eval_batch_size,
                        ...
                        evaluator_tokenizer=rm_model_tokenizer),
        logger=logger,
    )
    ...
    ppo_trainer = PPOTrainer(
        ...
        ckpt_saving_func=get_save_checkpoint_callback(path=args.policy_ckpt_path),
        ...
        enable_step_ppo = args.enable_step_ppo, # 开启Step level分步PPO
        enabling_bon = args.enabling_bon,                  # 开启 Best of N 经验池生成方式
