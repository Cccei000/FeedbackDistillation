...
from chatgpt.nn.llama import LlamaRM, modeling_llama_rm,LlamaFSRewardModel,LlamaRM_token,LlamaRM_multi
...
def add_neox_ppo_pipeline_args(parent_args:argparse.ArgumentParser):
    ...
    group.add_argument("--special_test_path", type=str, default= None)
...
def launch(args):
    ...
    special_test_dataset = ds.load_from_disk(args.special_test_path) if args.special_test_path else None
    ...
    if args.activation_checkpointing:
        ...
        print(f"Set Gradient Checkpointing: {model.is_gradient_checkpointing}")
    reward_model = LlamaRM_multi(
        model=model,
        value_head=None,
    ...
    collate_fn = {
        ...
        "special_test":RMCollator_MultiTurn(tokenizer,args.max_length, prefix_user=args.prefix_user, prefix_bot=args.prefix_bot),
    }
    # 初始化trainer
    # args.mode = "token"
    rm_trainer = RMTrainer(
        ...
        special_test_dataset=special_test_dataset,
        mode = args.mode,
    )
    # 开始训练
    rm_trainer.fit()
