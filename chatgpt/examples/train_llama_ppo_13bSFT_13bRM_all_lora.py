...
from fengshen_inner.models.llama.modeling_llama_lora import LlamaForCausalLMLora, LlamaModelLora
from fengshen_inner.models.megatron import mpu

from chatgpt.dataset import ReplayBufferDataset
from chatgpt.nn.llama import LlamaActor, LlamaCritic, modeling_fengshenLlama_rm,LlamaRM
...
def launch(args):
    ...
    ref_model = LlamaForCausalLM.from_pretrained(f"{args.policy_model_path}/part_{mpu.get_model_parallel_rank()}", torch_dtype=torch.bfloat16).to(dtype=torch.bfloat16)
    ...
    raw_reward_model = LlamaModelLora(actor_config, ref_model.llama).to(dtype=torch.bfloat16)
    # TODO: load训练好的RM lora 权重
    raw_reward_model.enable_lora()
    ...
    reward_model = LlamaRM(
            model=raw_reward_model,
            value_head=None,
        ).to(dtype=torch.bfloat16).eval().cpu()
