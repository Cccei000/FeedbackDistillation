...
from chatgpt.nn.llama import LlamaActor, LlamaCritic, modeling_llama_rm
...
def get_experience_convert_func(args, src_tokenizer:LlamaTokenizer, dst_tokenizer:LlamaTokenizer):
    ...
    def convert_func(sequences:torch.Tensor, attention_mask:torch.Tensor, action_mask:torch.Tensor, device:torch.device):
        ...
        try:
            sequence_texts = src_tokenizer.batch_decode(sequences, skip_special_tokens=False) # 完整的句子
            sequence_texts = [text.replace('<s>', '').replace('</s>', '') for text in sequence_texts]
        ...
        end_with_eos = torch.diagonal(sequences[:,last_index] == src_tokenizer.eos_token_id)
        ...
        query_texts = src_tokenizer.batch_decode(quert_ids, skip_special_tokens=False) # 仅query
        query_texts = [text.replace('<s>', '').replace('</s>', '') for text in query_texts]
        ...
        print_rank_0(f'sequence_texts {process_text(sequence_texts[0])} query_texts {process_text(query_texts[0])}')
        for idx, (seq_text, query_text) in enumerate(zip(sequence_texts, query_texts)):
            ...
            act_length = max(seq_len - len(query_encode_dict["input_ids"]), 1)
...
def launch(args):
    ...
    rm_model_tokenizer = LlamaTokenizer.from_pretrained(args.rm_model_tokenizer_path)
    ...
    reward_model = modeling_llama_rm(
        config_path=args.rm_config_path, ckpt_path=args.rm_model_path,\
        convert_func=get_experience_convert_func(args, llama_tokenizer, rm_model_tokenizer)
    ).eval().half().cpu()
